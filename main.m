...
solver_wrapper = py.importlib.import_module('solver_wrapper');

% init guilda

env_vars = utils.env('.env');
...
s_params = solver_wrapper.ScenarioParameters( ...
    M_1 = 100, ...
    D_1 = 10, ...
...
sol_params = solver_wrapper.SolverParameters( ...
    device = 'cuda', ...
    optim_factory = solver_wrapper.default_optim_factory, ...
    batch_size = 20, ...
    batch_time = 30, ...
    ode_params = py.dict(method = 'rk4') ...
);

model = solver_wrapper.EnvModelEstimator( ...
    scenario, ...
    sol_params ...
);
